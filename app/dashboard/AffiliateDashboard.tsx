...
function Sidebar({ affiliate, sidebarOpen, activeSection, onToggleSidebar, onSectionChange, onLogout }: any) {
  ...
  const handleClick = (itemId: string) => {
    if (itemId === 'quick-admission') {
      window.location.href = '/quick-admission';
    } else {
...
export default function AffiliateDashboard() {
  ...
  // ---------- Generate Admission Letters for Qualified Leads ----------
  const generateLettersForQualifiedLeads = async () => {
    const qualifiedLeads = leads.filter(l => l.qualified && l.status !== 'converted');
    if (qualifiedLeads.length === 0) {
      alert('No qualified leads found');
      return;
    }
    let successCount = 0;
      ...
      try {
        const res = await fetch('/api/admission/generate', {
          ...
          body: JSON.stringify({
            ...
            program: lead.program || 'General Program',
            mode_of_study: 'Full-time',
            institution_id: selectedInstitution || null,
            reference_person: affiliate?.full_name,
    ...
    alert(`Generated letters for ${successCount} out of ${qualifiedLeads.length} qualified leads`);
    loadAffiliateData();
  };
      ...
      <div className="lg:ml-72 min-h-screen">
        <main className="p-6">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Leads Section (with import button and generate letters button) */}
            {activeSection === 'leads' && (
              <div className="space-y-6">
                <div className="bg-white rounded-2xl p-6 shadow-sm border">
                  <div className="flex flex-wrap gap-3 items-center justify-between mb-6">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={generateLettersForQualifiedLeads}
                        className="px-3 py-2 bg-purple-600 text-white rounded-lg text-sm flex items-center gap-1"
                      >
                        <FileText size={16} /> Generate Letters for Qualified
                      </button>
                    </div>
                  </div>
                        ...
                        {paginatedLeads.map((lead) => (
                          <tr key={lead.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-4">
                              {lead.qualified ? (
                                <span className="inline-block px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs">Qualified</span>
                              ) : (
                                <span className="inline-block px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs">Rejected</span>
                              )}
                              {lead.grading_reason && (
                                <p className="text-xs text-gray-400 mt-1 max-w-xs">{lead.grading_reason.substring(0, 60)}</p>
                              )}
                            </td>
                            ...
                            <td className="px-4 py-4">
                              <select value={lead.status || 'new'} onChange={(e) => updateLeadStatus(lead.id, e.target.value)} className={`text-xs border rounded px-2 py-1 bg-white ${getStatusBadge(lead.status)}`}>
                            ...
                            <td className="px-4 py-4">
                              <div className="flex gap-1">
                                <button onClick={() => sendWhatsApp(lead)} className="p-1.5 bg-green-100 text-green-700 rounded"><MessageCircle size={14} /></button>
